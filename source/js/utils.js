...
NexT.utils = {
  ...
  registerCopyButton(target, element, code = "") {
    ...
    button.addEventListener("click", () => {
      if (!code) {
        const lines =
          element.querySelector(".code") || element.querySelector("code");
  ...
  registerCodeblock(element) {
    ...
    let figure;
    if (CONFIG.hljswrap) {
      figure = (inited ? element : document).querySelectorAll(
        "figure.highlight",
      );
    } else {
        ...
        if (backToTop || readingProgressBar) {
          ...
          if (backToTop) {
            if (
              ...
              !backToTop.classList.contains("back-to-top-on")
            )
  ...
  activateNavByIndex(index) {
    ...
    let activateEle =
      target.querySelector(".nav-child") || target.parentElement;
    ...
    const tocElement = document.querySelector(
      CONFIG.scheme === "Pisces" || CONFIG.scheme === "Gemini"
  ...
  activateSidebarPanel(index) {
    ...
    const panelHeights = [postTOCHeight, overviewPanel.scrollHeight];
    panelContainer.style.setProperty(
      "--inactive-panel-height",
      `${panelHeights[1 - index]}px`,
    );
    panelContainer.style.setProperty(
      "--active-panel-height",
      `${panelHeights[index]}px`,
    );

    sidebar.classList.replace(
      activeClassNames[1 - index],
      activeClassNames[index],
    );
  },
  ...
  registerTabsTag() {
    ...
    document.querySelectorAll(".tabs ul.nav-tabs .tab").forEach((element) => {
      element.addEventListener("click", (event) => {
        ...
        const activeTab =
          tabContent.querySelector(".active") || tabContent.firstElementChild;
        // Hight might be `auto`.
        const prevHeight =
          parseInt(window.getComputedStyle(activeTab).height, 10) || 0;
        const paddingTop = parseInt(
          window.getComputedStyle(activeTab).paddingTop,
          10,
        );
        const marginBottom = parseInt(
          window.getComputedStyle(activeTab.firstElementChild).marginBottom,
          10,
        );
        ...
        const currHeight = parseInt(
          window.getComputedStyle(tabContent.querySelector(".active")).height,
          10,
        );
        ...
        setTimeout(() => {
          if (
            ...
            const currHeightAfterScrollBarChange = parseInt(
              window.getComputedStyle(tabContent.querySelector(".active"))
                .height,
              10,
            );
            tabContent.style.height =
              currHeightAfterScrollBarChange + paddingTop + marginBottom + "px";
          }
        ...
        const offset =
          nav.parentNode.getBoundingClientRect().top + window.scrollY + 10;
        window.anime({
          targets: document.scrollingElement,
          duration: 500,
          easing: "linear",
  ...
  updateFooterPosition() {
    ...
    function updateFooterPosition() {
      ...
      const containerHeight =
        document.querySelector(".main").offsetHeight + footer.offsetHeight;
